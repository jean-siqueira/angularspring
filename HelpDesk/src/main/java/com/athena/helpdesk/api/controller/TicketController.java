...
import com.athena.helpdesk.api.dto.Summary;
...
public class TicketController {
	...
	public ResponseEntity<Response<Page<Ticket>>> findByParams(HttpServletRequest request, 
		...
		priority = title.equals("priority") ? "" : priority;
	...
	public ResponseEntity<Response<Ticket>> changeStatus(
		...
		try {
			validateUpdateStatus(id, status, result);
			if (result.hasErrors()) {
				result.getAllErrors().forEach(error -> {
					response.getErrors().add(error.getDefaultMessage());
				});
	...
	private void validateUpdateStatus(String id, String status,BindingResult result) {
		...
		if(tickets != null) {
			for (Iterator iterator = tickets.iterator(); iterator.hasNext();) {
				...
				if(ticket.getStatus().equals(StatusEnum.New)) {
					...
					amountAssigned ++;
				} else if(ticket.getStatus().equals(StatusEnum.Close)) {
					amountClosed ++;
				}
			}
